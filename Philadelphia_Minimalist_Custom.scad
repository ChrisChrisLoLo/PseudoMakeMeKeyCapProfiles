...
//  BotWid, BotLen, TWDif, TLDif, keyh, WSft, LSft  XSkew, YSkew, ZSkew,/*|*/ WEx, LEx, CapR0i, CapR0f, CapR1i, CapR1f, CapREx, StemEx, chop shift
    ...
    [14.50,  14.50, 3.0, 3.0,  2.5,  0.0,  0.0, 0.0, -0, -0,/*|*/ 2,   2,    1,      3,     1,      3,     2,       2, 0], //R2 Top surface  
...
//      difference(){
        ...
        //Bottom shell
        if( Stem == Choc){
        // CHANGED: I forced the eliptical rectangle to have dimensions 14.5 and 14.5
        skin([for (i=[0:layers]) transform(translation(CapTranslation(i, keyID+1)) * rotation(CapRotation(i, keyID+1)), elliptical_rectangle([14.5,14.5], b =  [1,1],fn=fn))]); //outer shell
        }else {
